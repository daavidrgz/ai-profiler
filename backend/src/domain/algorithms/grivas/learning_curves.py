...
from domain.algorithms.grivas15.entities.dataset import ProfilingDataset
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Train a model with crossvalidation"
    ...
    parser.add_argument(
        ...
        type=str,
        dest="recipe",
        help="path to the recipe to use, if not specified " "default recipe is used",
    )
    ...
    if task in tasks:
        ...
        print(
            'task "%s" does not exist - try one of the' " following: %s" % (task, tasks)
        )
