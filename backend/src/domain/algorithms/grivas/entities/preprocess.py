...
def collapse_repetitions(texts):
    ...
    collapsed = []
    for text in texts:
        collapsed_text = re.sub(r"([\w%s])\1+" % string.punctuation, r"\1\1", text)
...
def silhouette(texts):
    ...
    for text in texts:
        sil_text = re.sub(
            r"([\p{Lu}])(\1+)", lambda match: chr(31) * (len(match.group(2)) + 1), text
        )
        sil_text = re.sub(
            r"([\p{Ll}])(\1+)",
        ...
        sil_text = re.sub(r"(\p{Lu}+?)", "U", sil_text)
        sil_text = re.sub(r"(\p{Ll}+?)", "l", sil_text)
        sil_text = re.sub(r"(\d+?)", lambda match: "d", sil_text)
