...
CONF_FOLDERNAME = "./domain/algorithms/grivas/config"
...
class Config(object):
    ...
    def __init__(self, lang):
        ...
        with open(filename) as conf_file:
            ...
            self._settings = yaml.safe_load(conf_file)
    ...
    def truth_mapping(self):
        ...
        """
        return {
            pred: self._settings[pred]["column"]
    ...
    def tasks(self):
        ...
        return [
            label
            for label, values in self._settings.items()
            ...
            [
                ...
                for each in sorted(
                    [
                        (
                            pred,
                            self._settings[pred]["result_label"],
    ...
    def regression_list(self):
        ...
        return [
            label
            for label, values in self._settings.items()
            if ONLY_DATA not in values.keys() and values["type"] == REGRESSION
        ]
    ...
    def classifier_list(self):
        ...
        return [
            label
            for label, values in self._settings.items()
            if ONLY_DATA not in values.keys() and values["type"] == CLASSIFICATION
        ]
